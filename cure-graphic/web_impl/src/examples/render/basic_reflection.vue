 ...
 */
function intersec_ray_sphere(
    origin: CureVector3D,
    direction: CureVector3D,
    sphere: CureSphere,
): [number, number] {
    // 此部分内容需要绘图才能理解！
    ...
    for (const light of lights) {
        ...
        if (light.type === "ambient") {
            ...
            if (shadow_switcher.value) {
                const [shadow_sphere, _] = closest_intersection(
                    point,
                    light_vector,
                    0.001,
                    shadow_t_max,
            ...
            if (specular_switcher.value && specular > 0) {
                ...
                if (r_dot_v > 0) {
                    const intensity_r = Math.pow(
                        r_dot_v / (reflect.Length * direction.Length),
                        specular,
                    );
...
function trace_ray(
    ...
    const [closest_sphere, closest_t] = closest_intersection(
        origin,
        direction,
        t_min,
        t_max,
    ...
    const point = CureVector3D.add(
        origin,
        CureVector3D.mul(direction, closest_t),
    );
    // 计算该像素点的法线方向，因为它位于球体上，所以法线方向就是从球心指向交点的向量
    // 注意向量需要归一化！
    const normal = CureVector3D.sub(
        point,
        closest_sphere.position,
    ).normalized();
    // 计算该点的光照强度。注意这里的 direction 表示的是摄像机【看向该点】的方向向量，需要取反！
    // 这里需要获取【该点看向摄像机】的方向向量哟
    const intensity = compute_lighting(
        point,
        normal,
        direction.to_reverse(),
...
function on_change() {
    draw_canvas();
    drawTopView(top_view_canvas!, top_view_canvas_ctx!, {
        spheres: spheres.slice(0, 3),
        lights,
    });
    drawRightView(right_view_canvas!, right_view_canvas_ctx!, {
        spheres: spheres.slice(0, 3),
        lights,
    });
}
...
onMounted(() => {
    ...
    main(canvas);
    top_view_canvas = document.querySelector(
        ".top-view-canvas",
    ) as HTMLCanvasElement;
    top_view_canvas_ctx = top_view_canvas.getContext("2d");
    right_view_canvas = document.querySelector(
        ".right-view-canvas",
    ) as HTMLCanvasElement;
